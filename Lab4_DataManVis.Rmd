...
date: "Due by 11:59pm on Apr 23, 2024"
output:
...
3. Note: `str.url` is defined for this question and the next one; you can simply refer to it in your solution code (it is not part of the code you have to convert to pipes). 

As a backup, the data are also available to download on Canvas, Lab #4.

In addition, you can ignore the difference between the headers from the output of your pipes code and the original base R code.
...
As a backup, the data are also available to download on Canvas, under Lab #4.
...
```{r}
sprint_w = read.table(
  file="https://github.com/zhangyk8/zhangyk8.github.io/raw/master/_teaching/file_stat302/Data/sprint.w.txt", sep="\t", quote="", header=TRUE)
```
